...
struct InputState {
  // Rotation Sensor //
  vec3 firstRotationVector = vec3{0, 0, 0};
  vec3 lastRotationVector = vec3{0, 0, 0};
      ...
      if (pollResult == LOOPER_ID_USER) {
        if (engine.gameRotationSensor != nullptr && engine.sensorEventQueue != nullptr) {
          while (ASensorEventQueue_getEvents(engine.sensorEventQueue, &sensorEvent, 1) > 0) {
            ...
            engine.inputState.lastRotationVector = vec3{sensorEvent.vector.x, sensorEvent.vector.y,
...
static void update(Engine *engine) {
  ...
  sceneInput.x = inputState.deltaPos_normalized.x;
  sceneInput.y = inputState.deltaPos_normalized.y;

  if (inputState.lastRotationVector.x == 0 &&
      inputState.lastRotationVector.y == 0 &&
      inputState.lastRotationVector.z == 0) {
    // If the sensor hasn't given us anything our rotation is an identity matrix
    sceneInput.rotationMat = identity_mat3();
  } else if (inputState.firstRotationVector.x == 0 &&
             inputState.firstRotationVector.y == 0 &&
             inputState.firstRotationVector.z == 0) {
    // if this is our first time processing a rotation vector first rotation is last rotation
    inputState.firstRotationVector = inputState.lastRotationVector;
    sceneInput.rotationMat = identity_mat3();
  }
